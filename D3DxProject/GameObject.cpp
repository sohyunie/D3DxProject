...
void CAirplaneObject::SetShader(CShader* pShader)
{
	m_nMaterials = 1;
...
void CAirplaneObject::SetShader(int nMaterial, CShader* pShader)
{
	if (m_ppMaterials[nMaterial]) m_ppMaterials[nMaterial]->SetShader(pShader);
}

void CAirplaneObject::SetMaterial(int nMaterial, CMaterial* pMaterial)
{
...
void CAirplaneObject::SetChild(CAirplaneObject* pChild, bool bReferenceUpdate)
{
	if (pChild)
	{
...
void CAirplaneObject::Animate(float fTimeElapsed, XMFLOAT4X4* pxmf4x4Parent)
{
	if (m_pSibling) m_pSibling->Animate(fTimeElapsed, pxmf4x4Parent);
	if (m_pChild) m_pChild->Animate(fTimeElapsed, &m_xmf4x4World);
}

void CAirplaneObject::UpdateShaderVariable(ID3D12GraphicsCommandList* pd3dCommandList, XMFLOAT4X4* pxmf4x4World)
{
	XMFLOAT4X4 xmf4x4World;
...
void CAirplaneObject::UpdateShaderVariable(ID3D12GraphicsCommandList* pd3dCommandList, CMaterial* pMaterial)
{
}

void CAirplaneObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	OnPrepareRender();
	...
	if (m_nMaterials > 0)
	{
		for (int i = 0; i < m_nMaterials; i++)
		{
			...
			if (m_ppMeshes) m_ppMeshes[0]->Render(pd3dCommandList, i);
		}
	}
...
void CAirplaneObject::UpdateTransform(XMFLOAT4X4* pxmf4x4Parent)
{
...
void CAirplaneObject::SetPosition(float x, float y, float z)
{
...
void CAirplaneObject::SetScale(float x, float y, float z)
{
...
void CAirplaneObject::Rotate(float fPitch, float fYaw, float fRoll)
{
...
void CAirplaneObject::Rotate(XMFLOAT3* pxmf3Axis, float fAngle)
{
...
void CAirplaneObject::Rotate(XMFLOAT4* pxmf4Quaternion)
{
...
CMeshLoadInfo* CAirplaneObject::LoadMeshInfoFromFile(FILE* pInFile)
{
	char pstrToken[64] = { '\0' };
	UINT nReads = 0;
...
MATERIALSLOADINFO* CAirplaneObject::LoadMaterialsInfoFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, FILE* pInFile)
{
	char pstrToken[64] = { '\0' };
	UINT nReads = 0;
...
CAirplaneObject* CAirplaneObject::LoadFrameHierarchyFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, FILE* pInFile)
{
	char pstrToken[64] = { '\0' };
	UINT nReads = 0;
	...
	CAirplaneObject* pGameObject = NULL;
	...
	for (; ; )
	{
		...
		{
			pGameObject = new CAirplaneObject();
		...
		else if (!strcmp(pstrToken, "<Children>:"))
		{
			...
			if (nChilds > 0)
			{
				...
				{
					CAirplaneObject* pChild = CAirplaneObject::LoadFrameHierarchyFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pInFile);
...
void CAirplaneObject::PrintFrameInfo(CAirplaneObject* pGameObject, CAirplaneObject* pParent)
{
	TCHAR pstrDebug[256] = { 0 };
	...
	if (pGameObject->m_pSibling) CAirplaneObject::PrintFrameInfo(pGameObject->m_pSibling, pParent);
	if (pGameObject->m_pChild) CAirplaneObject::PrintFrameInfo(pGameObject->m_pChild, pGameObject);
}

CAirplaneObject* CAirplaneObject::LoadGeometryFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, const char* pstrFileName)
{
	FILE* pInFile = NULL;
	...
	for (; ; )
	{
		...
		{
			pGameObject = CAirplaneObject::LoadFrameHierarchyFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pInFile);
		}
	...
	CAirplaneObject::PrintFrameInfo(pGameObject, NULL);
#endif
...
void CRevolvingObject::Animate(float fTimeElapsed, XMFLOAT4X4* pxmf4x4Parent)
{
	...
	CAirplaneObject::Animate(fTimeElapsed, pxmf4x4Parent);
}
