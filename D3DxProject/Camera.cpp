...
void CSpaceShipCamera::Rotate(float x, float y, float z)
{
	if (m_pPlayer && (x != 0.0f))
	{
		...
		벡터이다.*/
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, xmf3Position);
...
void CThirdPersonCamera::SetLookAt(XMFLOAT3& xmf3LookAt)
{
	...
	XMFLOAT3 xmf3Position = m_pPlayer->GetUpVector();
	XMFLOAT4X4 mtxLookAt = Matrix4x4::LookAtLH(m_xmf3Position, xmf3LookAt, xmf3Position);
